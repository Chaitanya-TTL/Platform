...
type Props = { root: TreeNodeData; source: SourceType; comparison?: Record<string, NodeComparison>; search: string; selectedId?: string; onSelect: (node: TreeNodeData) => void; onFullScreen: () => void; requirementTraceEnabled?: boolean; requirementResult?: RequirementTraceResult | null; requirementFocus?: ReverseRequirementTraceResult | null };

export function BomThreeUniverseView({ root, source, comparison, search, selectedId, onSelect, onFullScreen, requirementTraceEnabled = false, requirementResult, requirementFocus }: Props) {
  ...
  const forcedIds = useMemo(() => new Set([...requirementIds, ...impactIds, ...searchResultIds]), [requirementIds, impactIds, searchResultIds]);
  ...
  }, [baseVisible, unrelated, matched, comparison, selectedOccurrenceId, fullGraph, forcedIds]);
...
type SceneProps = { graph: VisualBomGraph; layout: ReturnType<typeof layoutThreeBom>; selectedId: string | null; hoveredId: string | null; comparison?: Record<string, NodeComparison>; analysis: ThreeAnalysisSnapshot; requirementIds: Set<string>; impactIds: Set<string>; relations: ReturnType<typeof relationshipState>; labels: ThreeLabelMode; nodeSize: ThreeNodeSize; colorBy: ThreeColorMode; sizeBy: ThreeSizeMode; unrelated: ThreeUnrelatedMode; matched: ThreeMatchedMode; focusActive: boolean; cameraAction: ThreeCameraAction; cameraActionKey: number; onHover: (id: string | null) => void; onSelect: (id: string) => void; onToggle: (id: string) => void };
function Scene(props: SceneProps) {
  const controls = useRef<ElementRef<typeof OrbitControls>>(null);
  return <><ambientLight intensity={.82}/><directionalLight position={[10, 18, 12]} intensity={2.2}/><pointLight position={[-18, 2, -12]} intensity={58} color="#22d3ee" distance={100}/><fog attach="fog" args={["#020617", 70, 190]}/><gridHelper args={[180, 60, "#164e63", "#0f172a"]} position={[0, -12, 0]}/>{props.graph.edges.map((edge) => <Edge3 key={edge.id} edge={edge} {...props}/>) }{props.layout.nodes.map((node) => { const selected = node.id === props.selectedId, ancestor = props.relations.ancestorIds.has(node.id), descendant = props.relations.descendantIds.has(node.id), sibling = props.relations.siblingIds.has(node.id), related = !props.selectedId || selected || ancestor || descendant || sibling, matchedGhost = props.matched === "ghost" && props.comparison?.[node.sourceNodeId]?.status === "matched", dimmed = (props.selectedId && !related && props.unrelated !== "show") || matchedGhost || (props.focusActive && !props.requirementIds.has(node.id)); const showLabel = props.labels === "all" || (props.labels === "assemblies" && (node.isAssembly || node.isRoot)) || (props.labels === "branch" && (selected || ancestor || descendant)) || (props.labels === "smart" && (node.isRoot || node.isAssembly || selected || node.id === props.hoveredId || Boolean(props.selectedId && (ancestor || node.parentId === props.selectedId)))); return <Node3 key={node.id} node={node} selected={selected} hovered={node.id === props.hoveredId} ancestor={ancestor} descendant={descendant} comparison={props.comparison?.[node.sourceNodeId]} analysis={props.analysis} requirementLinked={props.requirementIds.has(node.id)} impactLinked={props.impactIds.has(node.id)} dimmed={dimmed} hidden={props.unrelated === "hide" && props.selectedId !== null && !related} showLabel={showLabel} size={props.nodeSize} colorBy={props.colorBy} sizeBy={props.sizeBy} onHover={props.onHover} onSelect={props.onSelect} onToggle={props.onToggle}/>; })}<CameraRig controls={controls} layout={props.layout} selectedId={props.selectedId} relations={props.relations} action={props.cameraAction} actionKey={props.cameraActionKey}/><GizmoHelper alignment="bottom-right" margin={[88, 88]}><GizmoViewport axisColors={["#ef4444", "#22c55e", "#3b82f6"]} labelColor="white"/></GizmoHelper><OrbitControls ref={controls} makeDefault enableDamping dampingFactor={.08} minDistance={4} maxDistance={280}/></>;
}
function Edge3({ edge, layout, selectedId, relations, requirementIds, impactIds, unrelated }: SceneProps & { edge: VisualBomEdge }) { const a = layout.byId[edge.sourceId]?.position, b = layout.byId[edge.targetId]?.position; if (!a || !b) return null; const ancestor = Boolean(selectedId && relations.ancestorIds.has(edge.sourceId) && relations.ancestorIds.has(edge.targetId)), descendant = Boolean(selectedId && (edge.sourceId === selectedId || relations.descendantIds.has(edge.sourceId)) && relations.descendantIds.has(edge.targetId)), requirement = requirementIds.has(edge.sourceId) || requirementIds.has(edge.targetId), impact = impactIds.has(edge.sourceId) || impactIds.has(edge.targetId), related = !selectedId || ancestor || descendant; const color = requirement ? "#a78bfa" : impact ? "#f59e0b" : ancestor ? "#a78bfa" : descendant ? "#22d3ee" : "#334155"; return <Line points={[a, b]} color={color} lineWidth={requirement || impact ? 3 : related ? (selectedId ? 2.4 : 1) : .5} transparent opacity={requirement || impact ? 1 : related ? (selectedId ? 1 : .48) : unrelated === "show" ? .22 : .025}/>; }
...
type NodeProps = { node: ReturnType<typeof layoutThreeBom>["nodes"][number]; selected: boolean; hovered: boolean; ancestor: boolean; descendant: boolean; comparison?: NodeComparison; analysis: ThreeAnalysisSnapshot; requirementLinked: boolean; impactLinked: boolean; dimmed: boolean; hidden: boolean; showLabel: boolean; size: ThreeNodeSize; colorBy: ThreeColorMode; sizeBy: ThreeSizeMode; onHover: (id: string | null) => void; onSelect: (id: string) => void; onToggle: (id: string) => void };
function Node3(props: NodeProps) {
  ...
  useFrame(({ clock }, delta) => { if (group.current) { const scale = props.selected || props.requirementLinked || props.impactLinked ? 1.28 : props.hovered ? 1.13 : 1; group.current.scale.lerp(new THREE.Vector3(scale, scale, scale), Math.min(1, delta * 10)); group.current.position.lerp(new THREE.Vector3(...props.node.position), Math.min(1, delta * 7)); group.current.visible = targetOpacity > .001; } if (pulse.current) pulse.current.scale.setScalar(1.05 + (Math.sin(clock.elapsedTime * 3) + 1) * .18); });
  const stop = <TEvent extends Event>(event: ThreeEvent<TEvent>) => event.stopPropagation();
  return <group ref={group} position={props.node.position}><mesh onPointerOver={(event) => { stop(event); props.onHover(props.node.id); }} onPointerOut={(event) => { stop(event); props.onHover(null); }} onClick={(event) => { stop(event); props.onSelect(props.node.id); }} onDoubleClick={(event) => { stop(event); if (props.node.isAssembly) props.onToggle(props.node.id); }}><sphereGeometry args={[radius, 24, 18]}/><meshStandardMaterial color={color} emissive={props.requirementLinked ? "#8b5cf6" : props.impactLinked ? "#f59e0b" : props.selected ? "#06b6d4" : color} emissiveIntensity={props.requirementLinked || props.impactLinked || props.selected ? .9 : .16} transparent opacity={targetOpacity}/></mesh><mesh><sphereGeometry args={[radius + .08, 18, 14]}/><meshBasicMaterial color={props.comparison ? statusColors[props.comparison.status] : "#94a3b8"} wireframe transparent opacity={props.dimmed ? .02 : props.comparison ? .65 : .12}/></mesh>{props.requirementLinked || props.impactLinked || props.selected ? <mesh ref={pulse}><sphereGeometry args={[radius + .22, 22, 16]}/><meshBasicMaterial color={props.requirementLinked ? "#a78bfa" : props.impactLinked ? "#f59e0b" : "#22d3ee"} wireframe transparent opacity={.72}/></mesh> : null}{props.node.isAssembly && props.node.childIds.length ? <Billboard position={[radius * .72, radius * .72, 0]}><Text fontSize={.24} color="#67e8f9" outlineWidth={.02} outlineColor="#020617">{`+${props.node.childIds.length}`}</Text></Billboard> : null}{props.showLabel && !props.dimmed ? <Billboard position={[0, radius + .55, 0]}><Text fontSize={props.node.isRoot ? .48 : props.node.isAssembly ? .31 : .24} color="#f8fafc" anchorX="center" anchorY="middle" outlineWidth={.025} outlineColor="#020617">{short(props.node.name, props.node.isAssembly ? 30 : 22)}</Text></Billboard> : null}</group>;
}
...
function findByPath(root: TreeNodeData, path: number[]) {
