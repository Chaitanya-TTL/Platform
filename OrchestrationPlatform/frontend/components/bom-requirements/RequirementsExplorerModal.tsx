...
export function RequirementsExplorerModal({ catalog }: { catalog: RequirementCatalogEntry[] }) {
  ...
  const [expandedId, setExpandedId] = useState<string | null>(trace.selectedRequirement?.revision.id ?? null);

  const filtered = useMemo(() => {
    const term = query.trim().toLowerCase();
    let entries = catalog.filter((entry) => {
      const sourceMatch = filter === "all" || entry.record.source === filter;
      const content = [entry.record.partName, entry.record.partId, entry.record.bomId,
        entry.revision.revision, entry.revision.title, entry.revision.description,
        entry.revision.author, entry.revision.status].join(" ").toLowerCase();
  ...
  return createPortal(
    ...
    >
      <header className="shrink-0 border-b border-slate-800 bg-[#0a1021] px-5 pb-4 pt-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-3 text-xl font-semibold">Requirements Explorer</h2>
          </div>
          <button type="button" onClick={closeRequirementsExplorer} aria-label="Close drawer"
        ...
        <div className="mt-3 flex gap-2 grid grid-cols-3 text-sm pb-1">
          {(["all", "teamcenter", "configit", "windchill", "sap"] as const).map((source) => (
            <button key={source} type="button" onClick={() => setFilter(source)}
          ...
          <button type="button" onClick={() => setLatestOnly((value) => !value)}
      ...
      <div ref={listRef}
        ...
        <div className="grid grid-cols-1 gap-3">
          {filtered.map((entry) => {
            ...
            const focused = trace.selectedRequirement?.revision.id === entry.revision.id;
            ...
            return (
              <article key={entry.revision.id}
                ...
                <button type="button" onClick={() => activate(entry)} className="w-full p-4 text-left">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-[20px] font-semibold text-white">{entry.record.partName}</p>
                      <p className="mt-1 text-[12px] font-semibold uppercase tracking-[.12em] text-cyan-300">Item ID {entry.record.partId}</p>
                    </div>
                    <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[8px] font-bold uppercase ${tones[entry.record.source]}`}>
                      {labels[entry.record.source]} · {entry.revision.revision}
                    </span>
                  </div>
                  <h3 className="mt-3 text-[14px] font-semibold leading-5 text-slate-100">{entry.revision.title}</h3>
                  <div className="mt-3 flex items-center justify-between border-t border-slate-800 pt-3">
                    <span className="text-[12px] capitalize text-slate-500">{entry.revision.status} · {entry.revision.createdAt}</span>
                    <span className="inline-flex items-center gap-1 text-[12px] font-semibold text-violet-300">
                      {expanded ? "Collapse" : "Trace in BOMs"}{expanded ? <IconChevronUp className="h-3.5 w-3.5" /> : <IconChevronDown className="h-3.5 w-3.5" />}
                    </span>
                  </div>
                </button>
                ...
                {expanded ? (
                  <div className="border-t border-slate-800 bg-slate-950/35 px-4 pb-4 pt-3">
                    <p className="text-[13px] leading-5 text-slate-300">{entry.revision.description}</p>
                    <div className="mt-4 space-y-2">
                      <p className="text-[11px] font-bold uppercase tracking-[.14em] text-slate-500">Linked BOM occurrences</p>
                      {occurrences.length ? occurrences.map((item) => (
                        <div key={`${item.source}-${item.nodeId}`} className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/65 px-3 py-2.5">
                          <span className="text-[13px] font-medium text-slate-200">{labels[item.source]}</span>
                          <span className={`text-[11px] font-bold uppercase ${item.relationship === "direct" ? "text-violet-300" : "text-indigo-300"}`}>{item.relationship === "direct" ? "Direct link" : "Corresponding part"}</span>
                        </div>
                      )) : <p className="rounded-xl border border-dashed border-slate-700 p-3 text-[12px] text-slate-500">No occurrence found in the loaded BOMs.</p>}
                    </div>
                    <button type="button" onClick={clearRequirementFocus}
          ...
          {!filtered.length ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-12 text-center text-sm text-slate-500">No matching requirements.</div> : null}
        </div>
      </div>
      ...
      <footer className="flex shrink-0 items-center justify-between border-t border-slate-800 bg-slate-950/60 px-5 py-3 text-[15px] text-slate-500">
        <span>{filtered.length} requirement{filtered.length === 1 ? "" : "s"}</span>
        <span>{trace.focus ? `${trace.focus.occurrences.length} linked occurrence${trace.focus.occurrences.length === 1 ? "" : "s"}` : "Select to trace"}</span>
      </footer>
    </motion.aside>,
    document.body,
  );
