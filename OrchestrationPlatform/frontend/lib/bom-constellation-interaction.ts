...
export function lowestCommonAncestor(graph: VisualBomGraph, ids: string[]) {
  ...
  const paths = ids.map((id) => {
    ...
    while (current) { nodes.unshift(current.id); current = current.parentId ? graph.byId[current.parentId] : undefined; }
    return nodes;
  });
...
export function expandedToDepth(graph: VisualBomGraph, depth: 1 | 2 | 3 | "all") {
  const limit = depth === "all" ? Number.POSITIVE_INFINITY : depth;
  return new Set(graph.nodes.filter((node) => node.isAssembly && node.level < limit).map((node) => node.id));
}
export function safeFileName(value: string) { return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "bom-constellation"; }
