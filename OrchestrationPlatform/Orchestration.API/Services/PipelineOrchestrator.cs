...
public sealed class PipelineOrchestrator:IPipelineOrchestrator
{
    ...
    public async Task<(bool success,BomRoot? finalBom,string outputFilePath,string outputKind)> ExecutePipelineAsync(string jobId,ExtractionRequest request,Func<PipelineProgress,Task> callback,CancellationToken cancellationToken=default)
    {
        ...
        try
        {
            ...
            if(!validResult)throw new Exception($"Pipeline execution failed: {extraction.Output}");
            ...
            if(isSapImpactRequest){var warnings=new List<string>();if(extraction.SapImpact?.Warnings!=null)warnings.AddRange(extraction.SapImpact.Warnings);if(extraction.SapHistory?.Warnings!=null)warnings.AddRange(extraction.SapHistory.Warnings);var fallback=extraction.SapHistory?.Fallback;var dataSource=fallback?.Active==true||warnings.Any(w=>w.Contains("FALLBACK_USED",StringComparison.OrdinalIgnoreCase))?"fallback-snapshot":"live";var warningCodes=warnings.Select(w=>w.Split(':',2)[0].Trim()).Where(w=>w.StartsWith("SAP_",StringComparison.Ordinal)).Distinct(StringComparer.Ordinal).ToList();audit.SapOperationalImpact=new SapOperationalImpactResult{SourceMaterialId=request.MaterialId??"",Plant=request.Plant??"1001",Status=dataSource=="fallback-snapshot"||extraction.Bom==null?"partial_fallback_success":extraction.SapImpact!=null&&extraction.SapHistory!=null&&warnings.Count==0?"complete":"partial_success",DataSource=dataSource,Fallback=fallback,WarningCodes=warningCodes,CurrentState=extraction.SapImpact,History=extraction.SapHistory,Warnings=warnings.Distinct(StringComparer.Ordinal).ToList(),GeneratedAt=DateTime.UtcNow.ToString("O")};}
        ...
        catch(Exception ex){_logger.LogError(ex,"Pipeline failed for job {JobId}",jobId);audit.Status="failed";audit.Error=ex.Message;audit.EndTime=DateTime.UtcNow;await _auditLogger.LogAsync(audit);await Report(jobId,"error","error",0,$"Error: {ex.Message}",callback,channel);return(false,null,"",outputKind);}
