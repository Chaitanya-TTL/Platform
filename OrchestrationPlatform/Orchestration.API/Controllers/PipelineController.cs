...
namespace Orchestration.API.Controllers
{
    ...
    public class PipelineController : ControllerBase
    {
        ...
        public PipelineController(IPipelineOrchestrator orchestrator, IAuditLogger auditLogger, IJobStore jobStore,
            ISubprocessExecutor subprocessExecutor, ILogger<PipelineController> logger)
        {
        ...
        {
            _logger.LogInformation("Received pipeline start request. Kind={Kind}, Identifier={Identifier}", request.Kind, request.GetIdentifier());
            ...
            if (request.Kind == ExtractionKind.Sap && string.IsNullOrWhiteSpace(request.MaterialId))
                return BadRequest(new { success = false, message = "MaterialId is required" });
            ...
            try
            {
                ...
                return Ok(new { success = true, jobId, kind = request.Kind.ToString().ToLowerInvariant(), message = "Extraction started successfully" });
            }
            catch (Exception ex)
            {
        ...
        public async Task StartProgressStream(string jobId)
        {
            ...
            {
                await Response.WriteAsync($"data: {System.Text.Json.JsonSerializer.Serialize(progress)}\n\n");
                await Response.Body.FlushAsync();
            });
        }

        [HttpGet("logs")]
        public async Task<IActionResult> GetAllLogs() => Ok(await _auditLogger.GetAllLogsAsync());

        [HttpGet("logs/{jobId}")]
        public async Task<IActionResult> GetLogByJobId(string jobId)
        {
            var log = await _auditLogger.GetLogByJobIdAsync(jobId);
            return log == null ? NotFound(new { error = $"No log found for job {jobId}", jobId }) : Ok(log);
        }
        ...
        {
            var log = await _auditLogger.GetLogByJobIdAsync(jobId);
            if (log == null) return NotFound(new { error = $"No log found for job {jobId}", jobId });
            if (log.FinalBom == null) return NotFound(new { error = $"No final BOM available for job {jobId}", jobId, status = log.Status });
        ...
        private async Task ExecutePipelineInBackground(string jobId, ExtractionRequest request)
        {
            try
            {
                var result = await _orchestrator.ExecutePipelineAsync(jobId, request, _ => Task.CompletedTask);
                if (result.success) _jobStore.CompleteJob(jobId);
                else _jobStore.FailJob(jobId, $"{request.Kind} extraction failed");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Pipeline execution failed for job {JobId}", jobId);
                _jobStore.FailJob(jobId, ex.Message);
            }
        }
    }
