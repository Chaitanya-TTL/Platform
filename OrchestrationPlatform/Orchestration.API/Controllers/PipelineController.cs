...
public class PipelineController:ControllerBase
{
    ...
    [HttpGet("status/{jobId}")]public IActionResult GetJobStatus(string jobId){var status=_jobStore.GetJobStatus(jobId);if(status==null)return NotFound(new{jobId,status="not_found",terminal=true});var failed=status.StartsWith("failed:",StringComparison.OrdinalIgnoreCase);return Ok(new{jobId,status,terminal=status=="completed"||failed,error=failed?status[(status.IndexOf(':')+1)..].Trim():null});}
